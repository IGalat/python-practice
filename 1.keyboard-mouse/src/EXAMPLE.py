...
tapper.group({"ctrl+y": "$(ctrl+shift+z)"}, "idea", trigger_if=lambda: Ct.get_fore(exec="idea"))
...
tapper.group(
    [
        ...
        Tap("num4", "$(rmb)"),
        Tap("num8", "$(alt+f4+f4)"),
        Tap("num0", lambda: messagebox.showinfo("mouse coords", str(Ct.get_mouse_pos()))),
    ],
    "mouse",
)
...
tapper.group(
    [
        Tap(
            ...
            trigger_if=lambda: Ct.get_open(exec="foobar") and Ct.get_fore(title="Youtube"),
        )
    ],
    "chrome",
