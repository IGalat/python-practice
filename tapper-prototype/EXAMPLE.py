...
def open_screen_in_paint():
    ...
    time.sleep(0.3)
    Ct.send("$(ctrl+v)$(esc)")  # paste, esc to remove select
    time.sleep(0.3)
    Ct.send("$(f11)")  # fullscreen
...
general.add(
    {  # doesn't work properly: app key opens menu on release. why?
        # "app": "_", "shift+-": "",
        # press f7 to collect mouse positions, then alt+f7 to copy list to clipboard, print and reset
        "alt+f7": finish_mouse_pos,
        "f7": add_mouse_pos,  # if swapped, alt+f7 won't work due to ordering
    }
)

# only work if foobar is open but not necessarily in the front, disable if it's not.
general.add(
    Tap("play_pause_media", "", trigger_if=lambda: not Ct.get_open(exec="foobar"))
)

tapper.group(
    {"ctrl+y": "$(ctrl+shift+z)"}, "idea", trigger_if=lambda: Ct.get_fore(exec="idea")
)
...
tapper.group(
    [
        ...
        Tap("num8", "$(alt+f4+f4)"),
        Tap(
            "num0", lambda: messagebox.showinfo("mouse coords", str(Ct.get_mouse_pos()))
        ),
    ],
    "mouse",
)
...
tapper.group(
    [
        Tap(
            "play_pause_media",
            youtube_and_music_swap,
            trigger_if=lambda: Ct.get_open(exec="foobar")
            and Ct.get_fore(title="Youtube"),
        )
    ],
    "chrome",
    trigger_if=lambda: Ct.get_fore(exec="chrome.exe"),
)
